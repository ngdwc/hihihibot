...
export async function handleThien(
  message: Message,
  args: string[],
  user: DbUser,
  ...
  if (sub === "start" || sub === "bat_dau") {
    if (isMeditating) {
      return new EmbedBuilder()
        .setColor("#FF4444")
        .setTitle("🧘 Đang thiền rồi!")
        .setDescription(
          "Bạn đang trong trạng thái thiền. Dùng `!thien stop` để dừng.",
        );
    }
  ...
  if (sub === "stop" || sub === "dung") {
    ...
    const earnedMonk = await checkAndGrant(
      user.discord_id,
      "monk",
      totalMinutes >= 60,
    );
    ...
    const earnedAngel = await checkAndGrant(
      user.discord_id,
      "angel",
      newVirtue >= 200,
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "⏱️ Thời gian thiền",
          value: `${minutesMeditated} phút`,
          inline: true,
        },
        {
          name: "✨ Công đức nhận được",
          value: `+${virtueGranted}`,
          inline: true,
        },
    ...
    if (earnedMonk)
      embed.addFields({
        name: "🏆 Thành tựu mới!",
        value: "🧘 **Đại Thiền Sư** — Thiền đủ 60 phút!",
      });
    if (earnedAngel)
      embed.addFields({
        name: "🏆 Thành tựu mới!",
        value: "😇 **Thiên Thần** — Công đức đạt 200!",
      });
    if (earnedDemon)
      embed.addFields({
        name: "🏆 Thành tựu mới!",
