...
import { DbUser, checkCooldown, setCooldown, addInventoryItem, addExp, hasItem } from '../db.js';
...
export const ORES: Ore[] = [
  ...
  { name: 'copper',  emoji: '🟤', displayName: 'Đồng',      chance: 25,   price: 500 },
  { name: 'iron',    emoji: '⚙️',  displayName: 'Sắt',       chance: 10,   price: 1_500 },
  { name: 'gold',    emoji: '🟡', displayName: 'Vàng',      chance: 5,    price: 8_000 },
  { name: 'diamond', emoji: '💎', displayName: 'Kim cương', chance: 2.5,  price: 50_000 },
  { name: 'emerald', emoji: '💚', displayName: 'Ngọc bích', chance: 1,    price: 200_000 },
];
...
export async function handleMine(message: Message, user: DbUser): Promise<EmbedBuilder> {
  ...
  if (remaining !== null) {
    return new EmbedBuilder().setColor('#FF4444').setTitle('⏰ Cuốc chưa sẵn sàng!')
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `Cooldown: 45 phút${hasPickaxe ? ' | ⛏️ Cuốc kim cương +1 quặng' : ''} • Bán: $sell ore <tên> <số lượng>` });
